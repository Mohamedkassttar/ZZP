...
export function Step6Summary({
  ...
  const fiscalProfit = Math.max(0, profit - investmentDeductionKIA);
  const totalDeductions = aovPremium + selfEmployedDeduction + starterDeduction;
  const taxableIncome = Math.max(0, fiscalProfit - totalDeductions);
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-6 space-y-6">
        ...
        <div className="space-y-3">
          ...
          <div className="flex justify-between items-center py-3 border-t border-gray-200">
            <span className="font-semibold text-gray-900">Winst uit Onderneming</span>
            <span className={`font-bold text-lg ${profit >= 0 ? 'text-green-600' : 'text-red-600'}`}>
              €{profit.toLocaleString('nl-NL', { minimumFractionDigits: 2 })}
            </span>
          </div>
          {investmentDeductionKIA > 0 && (
            <div className="flex justify-between items-center py-2">
              <div className="flex items-center gap-2">
                <CheckCircle className="w-4 h-4 text-emerald-600" />
                <span className="text-gray-700">Investeringsaftrek (KIA)</span>
              </div>
              <span className="font-semibold text-emerald-600">
                -€{investmentDeductionKIA.toLocaleString('nl-NL', { minimumFractionDigits: 2 })}
              </span>
            </div>
          )}
          <div className="flex justify-between items-center py-3 border-t border-gray-200 bg-blue-50 px-3 rounded">
            <span className="font-bold text-gray-900">Fiscale Winst</span>
            <span className={`font-bold text-xl ${fiscalProfit >= 0 ? 'text-blue-600' : 'text-red-600'}`}>
              €{fiscalProfit.toLocaleString('nl-NL', { minimumFractionDigits: 2 })}
            </span>
          </div>
        </div>
